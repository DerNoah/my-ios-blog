...
![The source image, before encoding](01-original.png){:width="280"}
*The source image we encode and decode throughout this guide.*
...
![The raw decoded BlurHash gradient](02-decoded.png){:width="280"}
*Step 2 — the raw decoded gradient, straight from the hash with no filtering.*
...
There is one case where they are *not* optional. Take a near-black subject on a white background — a product shot like this t-shirt. With only 4×3 coefficients the DCT can't represent that sharp dark-to-light edge, so the raw decode overshoots and clips the center to pure black: a **black hole**.

![A black-shirt-on-white product shot — a worst case for BlurHash](tshirt-original.png){:width="280"}
*The source: a near-black subject on white — the kind of high-contrast image that breaks a raw decode.*

![Raw BlurHash decode collapsing the dark center into a black hole](tshirt-raw.png){:width="280"}
*The raw decode — the over-dark center clips to pure black. No app should show this.*
...
![The decoded gradient after tonal compression](03-tonal.png){:width="280"}
*Step 3, Pass 1 — contrast pulled symmetrically toward mid-grey.*
...
![The final processed placeholder](04-blurred.png){:width="280"}
*Step 3, Pass 2 — tonal compression plus Gaussian blur: the final placeholder.*

Run those same two passes on the black-hole t-shirt and the failure case becomes presentable. Tonal compression lifts the clipped center out of pure black, and the blur dissolves its hard edge — the void becomes a calm grey placeholder. The lost detail isn't recovered (it never existed in the hash), but the alarming black hole is gone:

![The same t-shirt placeholder after tonal compression and blur](tshirt-processed.png){:width="280"}
*The same decode after Pass 1 + Pass 2 — the black hole is lifted into a neutral grey. For images like this, Step 3 isn't optional.*
...
When the real image arrives, snapshot the placeholder as an overlay and cross-dissolve it away. A `UIBlurEffect` layered on top rises to a peak halfway through the dissolve and then clears — the same frosted-glass shimmer the GIF above shows (blur ∝ `sin(π·t)`), rather than a blur that simply fades out from full to none:
...
func reveal(_ image: UIImage, in imageView: UIImageView) {
    ...
    let blurView = UIVisualEffectView(effect: nil)   // starts clear; peaks mid-transition
    blurView.frame = imageView.bounds
    blurView.autoresizingMask = [.flexibleWidth, .flexibleHeight]
    blurView.isUserInteractionEnabled = false

    imageView.image = image          // real image at the back
    imageView.addSubview(overlay)    // placeholder above it
    imageView.addSubview(blurView)   // frosted glass on top of the whole composite

    UIView.animateKeyframes(withDuration: 0.4, delay: 0, options: [.allowUserInteraction]) {
        // Cross-dissolve the placeholder away across the whole animation.
        UIView.addKeyframe(withRelativeStartTime: 0, relativeDuration: 1) {
            overlay.alpha = 0
        }
        // Frosted-glass shimmer: blur rises to a peak at the midpoint…
        UIView.addKeyframe(withRelativeStartTime: 0, relativeDuration: 0.5) {
            blurView.effect = UIBlurEffect(style: .systemUltraThinMaterial)
        }
        // …then clears by the end, matching the GIF's sin(π·t) shimmer.
